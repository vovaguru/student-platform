      ...
      <section className="attendance__head">
        <form className="w-full gap-4 accent-primary md:flex ">
          ...
          <div className="flex items-end">
            <button
              className="mb-4 h-10 rounded-md border-[1.5px] border-solid border-primary bg-primary px-8 py-2 font-semibold tracking-wide text-slate-200 hover:bg-violet-900 focus:bg-violet-900 disabled:cursor-not-allowed dark:border-violet-300 dark:bg-violet-900 dark:text-violet-100 dark:hover:bg-slate-900 md:w-auto"
              type="submit"
          ...
          {attendance?.length ? (
            <div className="my-4 w-full rounded-md border-2 border-slate-900 dark:border-slate-500 dark:p-[1px] lg:w-1/2">
              <table className="w-full text-center">
                <TableHeader Headers={["Час", "Занятие", "Посещаемость"]} />
                <tbody>
                  {attendance?.map((period, index) => (
                    ...
                    >
                      ...
                      <td className="p-2">
                        {period.attendance.present ? "Присутствовал" : "Отсутствовал"}
                      </td>
                    </tr>
                  ))}
                </tbody>
